...
const createCurriculumSchema = Joi.object({
  ...
  educationLevel: Joi.string().valid("primaire", "secondaire", "superieur").required().messages({
    "any.required": "Le niveau d'éducation est requis",
    "any.only": "Le niveau d'éducation doit être primaire, secondaire ou superieur",
  }),
    ...
    .items(
      Joi.object({
        ...
        topics: Joi.array()
          .items(
            Joi.object({
              ...
              learningObjectives: Joi.array().items(Joi.string().trim()).default([]),
              assessmentCriteria: Joi.array().items(Joi.string().trim()).default([]),
        ...
        assessments: Joi.array()
          .items(
            Joi.object({
              type: Joi.string().valid("formative", "summative").required().messages({
                "any.required": "Le type d'évaluation est requis",
                "any.only": "Le type doit être formative ou summative",
              }),
              weightage: Joi.number().min(0).max(100).optional(),
              criteria: Joi.array().items(Joi.string().trim()).default([]),
            }),
          )
          .default([]),
      }),
    ...
    Joi.object({
      ...
      topics: Joi.array().items(
        Joi.object({
          ...
          learningObjectives: Joi.array().items(Joi.string().trim()),
          assessmentCriteria: Joi.array().items(Joi.string().trim()),
          resourceIds: Joi.array().items(Joi.string()),
        }),
      ),
      assessments: Joi.array().items(
        Joi.object({
          type: Joi.string().valid("formative", "summative").required(),
          weightage: Joi.number().min(0).max(100),
          criteria: Joi.array().items(Joi.string().trim()),
        }),
      ),
    }),
  ),
...
const getCurriculaSchema = Joi.object({
  ...
  educationLevel: Joi.string().valid("primaire", "secondaire", "superieur").optional(),
  series: Joi.string().trim().optional(),
  isActive: Joi.boolean().default(true),
  sortBy: Joi.string().valid("country", "educationLevel", "createdAt").default("country"),
