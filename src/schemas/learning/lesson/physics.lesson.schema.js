...
const physicsLessonSchema = baseLessonSchema.append({
  subjectType: Joi.string().valid("physics").required().messages({
    "any.only": "Type de sujet doit être 'physics'",
    ...
    .items(
      Joi.object({
        ...
        topic: Joi.string()
          ...
          .required()
          .messages({
            "any.only": "Sujet de physique invalide",
          ...
          .items(
            Joi.object({
              name: Joi.string().trim().min(1).messages({
                "string.min": "Nom de la loi trop court (min 1 caractère)",
              }),
              formula: Joi.string().trim().min(1).messages({
...
const updatePhysicsLessonSchema = physicsLessonSchema.fork(
  [
    "topicId",
    "subjectType",
    "introduction",
...
const getPhysicsLessonSchema = Joi.object({
...
module.exports = {
  createPhysicsLessonSchema: physicsLessonSchema,
  updatePhysicsLessonSchema,
  getPhysicsLessonSchema,
};
