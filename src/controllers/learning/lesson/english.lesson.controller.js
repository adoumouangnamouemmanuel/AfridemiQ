const englishLessonService = require("../../../services/learning/lesson/english.lesson.service");
const { asyncHandler } = require("../../../utils/asyncHandler");
const { ApiError } = require("../../../utils/ApiError");
const createLogger = require("../../../services/logging.service");
const { Lesson } = require("../../../models/learning/lesson/lesson.base.model");

const logger = createLogger("EnglishLessonController");

class EnglishLessonController {
  // Create a new English lesson
  createLesson = asyncHandler(async (req, res) => {
    const lessonData = req.body;
    const result = await englishLessonService.createLesson(
      lessonData,
      req.user.id
    );
  ...
  getLessonById = asyncHandler(async (req, res) => {
    const { id } = req.params;
    const result = await englishLessonService.getLessonById(id);
    res.status(result.statusCode).json(result);
  });
  ...
  getLessons = asyncHandler(async (req, res) => {
    ...
    const result = await englishLessonService.getLessons(options);
    res.status(result.statusCode).json(result);
  });
  ...
  updateLesson = asyncHandler(async (req, res) => {
    ...
    if (
      ...
      throw new ApiError(
        403,
        "Non autorisé à mettre à jour cette leçon d'anglais"
      );
    }
    const result = await englishLessonService.updateLesson(
      id,
      req.body,
      req.user.id
    );
  ...
  deleteLesson = asyncHandler(async (req, res) => {
    ...
    if (
      ...
      throw new ApiError(403, "Non autorisé à supprimer cette leçon d'anglais");
    }
    const result = await englishLessonService.deleteLesson(id);
    res.status(result.statusCode).json(result);
  });
  ...
  addFeedback = asyncHandler(async (req, res) => {
    const { id } = req.params;
    const result = await englishLessonService.addFeedback(
      id,
      req.body,
      req.user.id
    );
...
module.exports = new EnglishLessonController();
