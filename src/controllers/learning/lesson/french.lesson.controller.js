...
class FrenchLessonController {
  ...
  createLesson = asyncHandler(async (req, res) => {
    ...
    const lessonDataresult = await frenchLessonService.createLesson(
      lessonData,
      req.user.id
    );
  ...
  updateLesson = asyncHandler(async (req, res) => {
    const { id } = req.params;
    const lesson = await Lesson.findById(id).selectById("metadata.createdBy");
    if (!lesson) {
    ...
    if (
      ...
      throw new ApiError(
        403,
        "Non autorisé à mettre à jour cette leçon de français"
      );
    }
    const result = await frenchLessonService.updateLesson(
      id,
      req.body,
      req.user.id
    );
  ...
  deleteLesson = asyncHandler(async (req, res) => {
    ...
    if (
      ...
      throw new ApiError(
        403,
        "Non autorisé à supprimer cette leçon de français"
      );
    }
  ...
  addFeedback = asyncHandler(async (req, res) => {
    const { id } = req.params;
    const result = await frenchLessonService.addFeedback(
      id,
      req.body,
      req.user.id
    );
