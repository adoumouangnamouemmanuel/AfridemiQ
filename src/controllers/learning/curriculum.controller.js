...
const getCurricula = async (req, res) => {
  try {
    ...
    logger.error("Error in getCurricula controller", error, { query: req.query })
    throw error
  }
}
...
const getCurriculumById = async (req, res) => {
  try {
    ...
    logger.error("Error in getCurriculumById controller", error, { curriculumId: req.params.id })
    throw error
  }
}
...
const addSubjectToCurriculum = async (req, res) => {
  try {
    const curriculum = await curriculumService.addSubjectToCurriculum(req.params.id, req.body)
    res.status(StatusCodes.OK).json({
...
const removeSubjectFromCurriculum = async (req, res) => {
  try {
    const curriculum = await curriculumService.removeSubjectFromCurriculum(req.params.id, req.params.subjectId)
    res.status(StatusCodes.OK).json({
...
const getCurriculaByCountry = async (req, res) => {
  try {
    const curricula = await curriculumService.getCurriculaByCountry(req.params.country)
    res.status(StatusCodes.OK).json({
      message: "Curricula récupérés par pays avec succès",
      data: curricula,
    })
  } catch (error) {
    logger.error("Error in getCurriculaByCountry controller", error, { country: req.params.country })
    throw error
  }
}
