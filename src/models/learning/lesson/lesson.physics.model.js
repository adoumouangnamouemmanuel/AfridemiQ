    ...
    concepts: [
      {
        ...
        topic: {
          type: String,
          enum: {
            values: PHYSICS_TOPICS,
            message: "Sujet de physique invalide",
          },
        ...
        laws: [
          {
            name: {
              type: String,
              trim: true,
              minlength: [1, "Nom de la loi trop court (min 1 caractère)"],
            },
            formula: {
              type: String,
              trim: true,
...
// Array validations
PhysicsLessonSchema.path("concepts").validate({
  validator: (v) => v.length > 0,
  message: "Au moins un concept est requis",
});
PhysicsLessonSchema.path("experiments").validate({
  validator: (v) => v.length > 0,
  message: "Au moins une expérience est requise",
});
PhysicsLessonSchema.path("practiceExercises").validate({
  validator: (v) => v.length > 0,
...
PhysicsLessonSchema.index({ prerequisites: 1 });
PhysicsLessonSchema.index({ "summary.suggestedNextTopics": 1 });
PhysicsLessonSchema.index({ "progressTracking.completedBy": 1 });
  ...
  try {
    ...
    if (quizIds.length > 0) {
      ...
      if (validQuizzes !== quizIds.length) {
        throw new Error("Identifiants de quiz invalides dans concepts");
      }
    }
